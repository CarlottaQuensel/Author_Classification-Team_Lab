...
class Evaluation():
    ...
    def __init__(self, gold_labels: list[str], predicted_labels: list[str]):
        ...
        Initialize evaluation with the gold labels
        and the predictions to be evaluated.
        ...
        # The gold and predicted labels should be for the same documents,
        # thus be the same number and order
        ...
        # Set the labels' numbering for consistent
        # error counting and confusion matrix
    ...
    def set_predictions(self, gold_labels, predicted_labels):
        ...
        if len(gold_labels) != len(predicted_labels):
            raise IndexError("Unequal amount of labelled instances for gold and predicted labels. Use set_predictions to set again.")
        # If precision, recall and f-score were computed once, reset them as
        # they don't match the current labels anymore
    ...
    def setConfusionMatrix(self, gold_labels=None, predicted_labels=None):
        ...
        # The CM can use the initial label set, an entirely new set or new
        # predictions for the same gold labels
    ...
    def precision(self, label: str = None, full_eva: bool = False) -> list[float]:
        """
        Author: Katrin Schmidt
        Method that computes precision for one or all labels
        as the ratio of predicted labels that are true.
        ...
        # If the user wants to see the precision for a specific author,
        # only compute this value
        if label:

            # Find label in the label dictionary and
            # compute precision for the respective class
            try:
            ...
            # Compute precision as the ratio of
            # correct classifications in all predicted instances
            try:
            ...
            # If no instances were assigned to the label,
            ...
            # When a full evaulation is performed, the results are passed on
            # to the full evaluation function to be shown together with the
            # other scores.
            if full_eva:
                self.precision_scores = prec_scores
                return prec_scores

            # Show precision for each label and their average (micro-F1)
            prec_print = 20*"_" + "\nPrecision:\n"
            p1 = ""
            p2 = ""
    ...
    def recall(self, label=None, full_eva: bool = False) -> list[float]:
        """
        Author: Katrin Schmidt
        Method that computes the recall for one or all labels
        as the ratio of gold labels that are predicted and show the scores.
        ...
        # List all individual recall scores to eventually
        # compute the average
        else:
            rec_scores = list()
    ...
    def f_score(self, label: str = None, beta: int = 1, full_eva: bool = False) -> list[float]:
        ...
        if label:
            ...
            # Compute the recall as the ratio of gold instances of the label
            # found by the classifier
            else:
                try:
        ...
        # If there are already calculated f-scores with the given beta value,
        # just print and return them
        else:
            if len(self.f_scores) and beta in self.f_scores:
                if full_eva:
    ...
    def showConfusionMatrix(self, full_eva: bool = False) -> dict[dict[int]]:
        ...
        # As the 20-40 author names are too long to use for a clear and neat table,
        # they are replaced by A1,...An with a table caption showing the mapping
        c_labels = len(self.labels)
        caption = "\n".join([f'A{i:<2} : {self.labels[i]}' for i in range(c_labels)])

        # The string for the label columns is as long as all labels combined
        # with a spacer in between each label.
        row_title = len("Gold: ")
        col_len = 4*c_labels

        # The column title of the table consists of the axis description as
        # predicted labels and the shortened author IDs (A1, A2...),
        # which are delimited by | as a vertical line
        ...
        # Each gold label has its own entry in a dictionary and its own row in
        # the printed table
        cm = dict()
        for gold_index in range(c_labels):
            ...
            for predicted_index in range(c_labels):
                # Adding the number of documents for which the gold (row) label
                # was predicted as the current (column) label to the output
        ...
        # After the whole matrix is traversed, it is printed in a readable format
        # and returned as an interpretable dictionary with the respective labels.
        # If the confusion matrix is shown as part of a full evaluation, the
        # string of the table is returned to the fullEval function to be printed
    ...
    def fullEval(self) -> dict[dict[int]]:
        ...
        # The micro average of the precision, recall and (due to beta=1/harmonic mean)
        # f-scores corresponds to the accuracy for this single-label multi-class classifier,
        # and is computed as (correct classifications / all classifications):
        ...
        # To print the scores of all labels and micro/macro average, the longest
        # author name has to be determined
